    ...
    >
      <div className="w-10 text-center font-bold text-lg">
        {rank <= 3 ? (
          ...
          <span className="text-gray-400">{getRankDisplay()}</span>
        )}
      </div>
      ...
      <div className="flex-1 min-w-0">
        <p className={`font-semibold truncate ${isCurrentUser ? 'text-primary-700' : 'text-gray-900'}`}>
          {profile.display_name}
          {isCurrentUser && <span className="text-xs ml-1 text-primary-500">(You)</span>}
        </p>
